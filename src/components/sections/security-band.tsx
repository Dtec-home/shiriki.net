...
export const FALLBACK_SECURITY_BAND: SecurityBandProps = {
  eyebrow: 'Security',
  heading: 'Members sign in with a phone number and a one-time code.',
  lead: 'No passwords to forget, reuse, or leak. Every admin action is written to an audit log your treasurer and board can read.',
  badges: [
    { icon: 'shield', label: 'OTP-only auth' },
    { icon: 'lock', label: 'Rotating JWTs' },
    { icon: 'database', label: 'Immutable audit logs' },
    { icon: 'globe', label: 'Per-church data isolation' },
  ],
  ussd: {
    eyebrow: 'USSD giving',
    code: USSD_CODE,
    text: 'The USSD channel runs on feature phones — no internet, no app download. Members register, give, and request an SMS statement from the handset they already own.',
    ctaLabel: 'View USSD simulator',
  },
}

/**
 * Security band: a 2x2 badge grid on the left, the USSD feature-phone panel
 * on the right. Anchored as `#security`.
 */
      ...
      <Container className="grid items-center gap-14 lg:grid-cols-2">
        ...
        <Reveal delay={0.08}>
          <div className="rounded-3xl bg-primary p-8 text-primary-foreground shadow-brand-xl lg:p-10">
            ...
            <SectionLabel className="mt-8 text-secondary dark:text-primary-foreground">{ussd.eyebrow}</SectionLabel>
